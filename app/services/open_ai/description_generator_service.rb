...
module OpenAi
  class DescriptionGeneratorService # rubocop:disable Metrics/ClassLength
    ...
    def initialize(product, title: nil, description: nil, categories: [])
      @product = product
      @store = product.store
      @custom_title = title
    ...
    attr_reader :product, :store, :custom_title, :custom_description, :categories
    ...
    def build_system_prompt # rubocop:disable Metrics/MethodLength
      base_prompt = [
        ...
        "3. If you don't recognize the product or lack sufficient information, respond with null for title, description and category_id",
        ...
        '  "category_id": "uuid of the most relevant subcategory or null"',
        '}'
      ].join("\n")
    ...
    def build_user_prompt # rubocop:disable Metrics/AbcSize
      ...
      prompt_parts << build_available_categories if categories.any?
    ...
    def build_available_categories
      lines = categories.map do |cat|
        parent_name = cat.category&.name || 'Root'
        "  - id: #{cat.id} | #{parent_name} > #{cat.name}"
      end
      "Available subcategories (pick the most relevant one):\n#{lines.join("\n")}"
    end
